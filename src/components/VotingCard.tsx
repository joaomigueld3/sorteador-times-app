...
export default function VotingCard({ player, onVoteChange, isSelf }: VotingCardProps) {
  ...
  // Cores dinâmicas para posições (adaptadas ao tema via opacidade)
  ...
  return (
    ...
    >
      {/* HEADER COMPACTO (Padding reduzido p-2) */}
            ...
            <div className="flex items-center gap-1 overflow-hidden">
              ...
              <div className="flex gap-0.5 shrink-0">
                {player.positions.map((pos) => (
                  <span key={pos} className={`text-[9px] font-bold px-1 rounded border ${getPositionColor(pos)}`}>
                    {pos}
                  </span>
                ))}
              </div>
      ...
      {isExpanded && (
        <div className="p-2 border-t space-y-2 bg-black/5" style={{ borderColor: 'var(--border)' }}>
          <CompactRow label="FIS (40%)" icon={<Activity size={12}/>} val={stats.fisico} onChange={(v) => handleUpdate({...stats, fisico: v})} />
          <CompactRow label="HAB (35%)" icon={<Zap size={12}/>} val={stats.habilidade} onChange={(v) => handleUpdate({...stats, habilidade: v})} />
          <CompactRow label="DEF (25%)" icon={<Shield size={12}/>} val={stats.defesa} onChange={(v) => handleUpdate({...stats, defesa: v})} />
        </div>
      )}
    </div>
  );
...
const CompactRow = ({ label, icon, val, onChange }: any) => (
  <div className="flex items-center gap-2 text-xs">
    <div className="flex items-center gap-1 w-20 shrink-0" style={{ color: 'var(--text-secondary)' }}>
    ...
    <input
      ...
      className="flex-1"
    />
    <span className="w-6 text-right font-medium" style={{ color: 'var(--text-primary)' }}>{val}</span>
  </div>
);